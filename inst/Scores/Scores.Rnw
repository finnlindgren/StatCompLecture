...
ggplot() + geom_point(aes(x_obs, y_obs))
@
...
\subsection{Distribution of score differences (new version)}
...
Expectation and variance of $S_\text{SE}(F^A,y)-S_\text{SE}(F^B,y)$ under $y\sim G$:
\begin{align}
S_\text{SE}(F^A,y)-S_\text{SE}(F^B,y) &= (y-\mu_A)^2 - (y-\mu_B)^2
\\&= -2y(\mu_A-\mu_B) + \mu_A^2 - \mu_B^2
\\&= (\mu_A+\mu_B - 2y)(\mu_A-\mu_B)
\\&= 2\left(y-\frac{\mu_B+\mu_A}{2}\right)(\mu_B-\mu_A) \\
\pE_{y\sim G}\left[S_\text{SE}(F^A,y)-S_\text{SE}(F^B,y)\right] &=
2\left(\mu_G-\frac{\mu_B+\mu_A}{2}\right)(\mu_B-\mu_A) \\
\pVar_{y\sim G}\left[S_\text{SE}(F^A,y)-S_\text{SE}(F^B,y)\right] &=
4\sigma_G^2 (\mu_B-\mu_A)^2
\end{align}
...
Expectation and variance of $S_\text{DS}(F^A,y)-S_\text{DS}(F^B,y)$ under $y\sim G$:
\begin{align}
S_\text{DS}(F^A,y)-S_\text{DS}(F^B,y) &=
\frac{(y-\mu_A)^2}{\sigma_A^2} - \frac{(y-\mu_B)^2}{\sigma_B^2} +
\log\left(\frac{\sigma_A^2}{\sigma_B^2}\right)
\\&=
\frac{(y-\mu_G)^2 + 2(y-\mu_G)(\mu_G-\mu_A)+(\mu_G-\mu_A)^2}{\sigma_A^2}
\\&\phantom{=}-
\frac{(y-\mu_G)^2 + 2(y-\mu_G)(\mu_G-\mu_B)+(\mu_G-\mu_B)^2}{\sigma_B^2} -
\log\left(\frac{\sigma_A^2}{\sigma_B^2}\right)
\\
\pE_{y\sim G}\left[S_\text{DS}(F^A,y)-S_\text{DS}(F^B,y)\right] &=
\frac{\sigma_G^2 + (\mu_G-\mu_A)^2}{\sigma_A^2}
-
\frac{\sigma_G^2 + (\mu_G-\mu_B)^2}{\sigma_B^2}
+ \log\left(\frac{\sigma_A}{\sigma_B^2}\right)
\end{align}
For the variance,
\begin{align}
\pVar_{y\sim G}&\left[S_\text{DS}(F^A,y) - S_\text{DS}(F^B,y)\right] \\
&=
\pE_{y\sim G}\left\{
\left[
\frac{(y-\mu_G)^2 + 2(y-\mu_G)(\mu_G-\mu_A)}{\sigma_A^2}
-
\frac{(y-\mu_G)^2 + 2(y-\mu_G)(\mu_G-\mu_B)}{\sigma_B^2}
-
\frac{\sigma_G^2}{\sigma_A^2}
+
\frac{\sigma_G^2}{\sigma_B^2}
\right]^2
\right\}
\\&=
\pE_{y\sim G}\left\{
\left[
(y-\mu_G)^2\left(\frac{1}{\sigma_A^2}-\frac{1}{\sigma_B^2}\right) +
2(y-\mu_G)\left(\frac{\mu_G-\mu_A}{\sigma_A^2} - \frac{\mu_G-\mu_B}{\sigma_B^2}\right)
-
\sigma_G^2\left(\frac{1}{\sigma_A^2}-\frac{1}{\sigma_B^2}\right)
\right]^2
\right\}
\\&=
\text{[$\sigma_{AB}=1/\sigma_A^2-1/\sigma_B^2$, $\mu_{AB}=\frac{\mu_A}{\sigma_A^2} - \frac{\mu_B}{\sigma_B^2}$]}
\\&=
\pE_{y\sim G}\left[
(y-\mu_G)^4 \sigma_{AB}^2
\right]
\\&\phantom{= }
+
\pE_{y\sim G}\left\{
(y-\mu_G)^3
4\sigma_{AB}(\mu_G\sigma_{AB}-\mu_{AB})
\right\}
\\&\phantom{=}+
\pE_{y\sim G}\left\{
(y-\mu_G)^2
\left[
4(\mu_G\sigma_{AB}-\mu_{AB})^2
-
2\sigma_G^2\sigma_{AB}^2
\right]
\right\}
\\&\phantom{= }
-
\pE_{y\sim G}\left\{
4(y-\mu_G)
(\mu_G\sigma_{AB}-\mu_{AB})\sigma_G^2\sigma_{AB}
\right\}
\\&\phantom{= }
+
\sigma_G^4\sigma_{AB}^2
\end{align}
Define the skewness $s_G=\pE\{\frac{(y-\mu_G)^3}{\sigma_G^3}\}$ and kurtosis
$k_G=\pE\{\frac{(y-\mu_G)^4}{\sigma_G^4}\}\geq s_G^2+1$.
Then
\begin{align}
\pVar_{y\sim G}&\left[S_\text{DS}(F^A,y) - S_\text{DS}(F^B,y)\right] \\
&=
k_G\sigma_G^4\sigma_{AB}^2
+
4s_G\sigma_G^3
\sigma_{AB}(\mu_G\sigma_{AB}-\mu_{AB})
\\&\phantom{=}+
\sigma_G^2
\left[
4(\mu_G\sigma_{AB}-\mu_{AB})^2
